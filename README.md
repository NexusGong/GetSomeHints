...
| 现象 | 说明 |
|------|------|
| **搜索结果与关键词不完全相关** | 抖音/小红书等使用平台官方搜索接口，返回结果由平台排序（综合/推荐等），可能混入热门、相关推荐或弱相关内容，属平台行为，非本工具过滤问题。可尝试更精确的关键词或时间范围。 |
| **日志里「正在搜索第 N 条」** | 该行为进度条，同一行原地刷新；最终停在「第 N 条」即本页/本次共抓取 N 条。前端若按 replace_id 去重，也只会保留最后一条。 |
| **「第 1 页」「第 2 页」** | 日志中的页码已按 1-based 修正：第 1 页即第一页结果，第 2 页无结果表示第二页接口未返回数据。 |
