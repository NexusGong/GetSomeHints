<div align="center">
  ...
  **GetSomeHints** — 多平台内容搜索与线索分析工具
  ...
  [环境](#环境) · [运行](#运行) · [API 契约](#api-契约) · [大模型分析](#大模型分析)
...
GetSomeHints 支持按关键词与条件对多平台进行内容抓取（帖子、视频、图文、评论），并内置代理池与防封策略。支持**大模型分析**：对爬取结果按场景（潜在买家/卖家、潜在热销品、热度话题）调用 DeepSeek 做线索与趋势分析。

| 项目 | 说明 |
|------|------|
| **当前爬虫** | 抖音、小红书采用 Playwright + 登录态 + 代理池，安装 Playwright 后为真实抓取；其余平台为示例桩。 |
| **技术栈** | 后端 Python FastAPI，前端 React + Vite + TypeScript，像素风 UI。 |
